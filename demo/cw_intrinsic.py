...
def timing_projectors(psrs, sigma_s: float):
    ...
    for psr in psrs:
        ...
        q, _ = np.linalg.qr(weighted, mode="reduced")
...
def sky_statistic_map(
    ...
    """Vectorised coherent 2F over many sky positions at one frequency."""
    temporal_rhs = np.empty((len(psrs), 2))
    temporal_gram = np.empty((len(psrs), 2, 2))
    inv_var = sigma_s**-2
    for p, (time, data) in enumerate(zip(times, data_blocks)):
        ...
        if operators is not None:
            temporal = operators[p] @ temporal
        temporal_rhs[p] = inv_var * temporal.T @ data
        temporal_gram[p] = inv_var * temporal.T @ temporal

    if sky_patterns is None:
        fp = np.empty((len(sky_theta), len(psrs)))
        fc = np.empty_like(fp)
        for s, (theta, phi) in enumerate(zip(sky_theta, sky_phi)):
            fp[s], fc[s] = antenna_patterns(psrs, theta, phi)
    else:
        fp, fc = sky_patterns
    ...
    normal = np.empty((len(sky_theta), 4, 4))
    for block_row, (left, _) in enumerate(((fp, fp), (fc, fc))):
        for block_col, (_, right) in enumerate(((fp, fp), (fc, fc))):
            weights = left * right
            for i in range(2):
                for j in range(2):
                    normal[:, 2 * block_row + i, 2 * block_col + j] = (
                        weights @ temporal_gram[:, i, j]
                    )
    inverse = np.linalg.pinv(normal, rcond=1e-12)
    amplitudes = np.einsum("sij,sj->si", inverse, rhs)
    values = np.einsum("si,si->s", rhs, amplitudes)
    return values
